...
# 52 "MCAL_Layer/USART/../Interrupt/mcal_interrupt_config.h"
typedef enum{
...
static void ESUART_ASYNC_RX_Init(const usart_t *_esuart);
    ...
    static void (* ESUART_TX_InterruptHandler)(void) = ((void*)0);


    static void (* ESUART_RX_InterruptHandler)(void) = ((void*)0);
    static void (* ESUART_FramingErrorInterruptHandler)(void) = ((void*)0);
    static void (* ESUART_OverrunErrorInterruptHandler)(void) = ((void*)0);
...
static void ESUART_Baud_Rate_Calculation(const usart_t *_esuart){
    ...
    switch(_esuart->baudrate_gen_config){
        case BAUDRATE_ASYN_8BIT_LOW_SPEED :
            ...
            TXSTAbits.BRGH = 0;
            BAUDCONbits.BRG16 = 0;
            Baud_Rate_Temp = ((8000000UL / (float)_esuart->baudrate) / 64) - 1;
            break;
        case BAUDRATE_ASYN_8BIT_HIGH_SPEED :
            TXSTAbits.SYNC = 0;
            TXSTAbits.BRGH = 1;
            BAUDCONbits.BRG16 = 0;
            Baud_Rate_Temp = ((8000000UL / (float)_esuart->baudrate) / 16) - 1;
            break;
        ...
        case BAUDRATE_ASYN_16BIT_HIGH_SPEED :
            ...
            TXSTAbits.BRGH = 1;
            BAUDCONbits.BRG16 = 1;
            Baud_Rate_Temp = ((8000000UL / (float)_esuart->baudrate) / 4) - 1;
            break;
...
static void ESUART_ASYNC_TX_Init(const usart_t *_esuart){
    if(1 == _esuart->usart_tx_cfg.usart_tx_enable){

        TXSTAbits.TXEN = 1;

        if(1 == _esuart->usart_tx_cfg.usart_tx_interrupt_enable){
            PIE1bits.TX1IE = 1;
...
# 213 "MCAL_Layer/USART/hal_usart.c"
        ...
        else if(0 == _esuart->usart_tx_cfg.usart_tx_interrupt_enable){
            PIE1bits.TX1IE = 0;
        }
        else{ }

        if(1 == _esuart->usart_tx_cfg.usart_tx_9bit_enable){
            TXSTAbits.TX9 = 1;
        }
        else if(0 == _esuart->usart_tx_cfg.usart_tx_9bit_enable){
            TXSTAbits.TX9 = 0;
        }
        else{ }
    }
...
static void ESUART_ASYNC_RX_Init(const usart_t *_esuart){
    if(1 == _esuart->usart_rx_cfg.usart_rx_enable){

        RCSTAbits.CREN = 1;

        if(1 == _esuart->usart_rx_cfg.usart_rx_interrupt_enable){
            PIE1bits.RCIE = 1;
...
# 259 "MCAL_Layer/USART/hal_usart.c"
        ...
        else if(0 == _esuart->usart_rx_cfg.usart_rx_interrupt_enable){
            PIE1bits.RCIE = 0;
        }
        else{ }

        if(1 == _esuart->usart_rx_cfg.usart_rx_9bit_enable){
            RCSTAbits.RX9 = 1;
        }
        else if(0 == _esuart->usart_rx_cfg.usart_rx_9bit_enable){
            RCSTAbits.RX9 = 0;
        }
        else{ }
    }
